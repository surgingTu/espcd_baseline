...
### Novel view rendering with explicit cameras (CUDA GPU only)

If you already have **source** and **target** camera intrinsics/extrinsics, you can directly render a novel-view image from a single input image:

```
sharp novel-view \
  -i /path/to/input.jpg \
  -o /path/to/output.png \
  --src-K /path/to/src_K.txt \
  --src-extrinsics /path/to/src_w2c.txt \
  --tgt-K /path/to/tgt_K.txt \
  --tgt-extrinsics /path/to/tgt_w2c.txt
```

Notes:
- Extrinsics are **world-to-camera (w2c)** by default. If you have camera-to-world matrices, add `--src-extrinsics-type c2w` / `--tgt-extrinsics-type c2w`.
- Intrinsics files can be 3x3 or 4x4; extrinsics files can be 3x4 or 4x4.
